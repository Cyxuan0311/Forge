...
static void test_gemv_q5_0() {
    ...
    for (int nb : {1, 4, 8}) {
        ...
        for (int N : {1, 3, 5, 8}) {
            ...
            std::vector<float> out_vsx(N), out_ref(N, 0.0f);
            forge::cpu::gemv_q5_0_transB_vsx(a.data(), w_blocks.data(), out_vsx.data(), 1, K, N);
            ...
            check_close_f32("gemv_q5_0_transB", out_vsx.data(), out_ref.data(), N, 1e-4f);
        }
    }
}
...
static void test_gemv_q5_1() {
    ...
    for (int nb : {1, 4, 8}) {
        ...
        for (int N : {1, 3, 5, 8}) {
            ...
            std::vector<float> out_vsx(N), out_ref(N, 0.0f);
            forge::cpu::gemv_q5_1_transB_vsx(a.data(), w_blocks.data(), out_vsx.data(), 1, K, N);
            ...
            check_close_f32("gemv_q5_1_transB", out_vsx.data(), out_ref.data(), N, 1e-4f);
        }
    }
}
