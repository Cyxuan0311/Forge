...
public:
    ...
    std::unique_ptr<InferenceEngine> create(const std::string& arch, Model& model, InferenceContext& ctx);
...
#define NANOINFER_REGISTER_ENGINE_IMPL2(line, arch, creator) \
    static ::nanoinfer::EngineAutoRegister _engine_reg_##line(arch, creator)

#define NANOINFER_REGISTER_ENGINE_IMPL(line, arch, creator) \
    NANOINFER_REGISTER_ENGINE_IMPL2(line, arch, creator)

#define NANOINFER_REGISTER_ENGINE(arch, creator) \
    NANOINFER_REGISTER_ENGINE_IMPL(__LINE__, arch, creator)
