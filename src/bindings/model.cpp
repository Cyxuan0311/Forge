...
py::dict PyModel::generate(py::array_t<int32_t, py::array::c_style> prompt_ids,
    ...
    if (buf.ndim != 1) throw std::runtime_error("prompt_ids must be 1D");
...
void register_model(py::module_& m) {
    py::class_<PyModel>(m, "Model")
        ...
        .def("load", &PyModel::load,
             ...
             py::arg("num_layers"),
             py::arg("num_heads"),
             py::arg("num_kv_heads") = 0,
             py::arg("head_dim") = 0,
             py::arg("rope_theta") = 10000.0f,
             py::arg("rms_norm_eps") = 1e-6f,
             py::arg("max_seq_len") = 4096,
             py::arg("arch_type") = "llama",
             py::arg("norm_type") = "rmsnorm",
             py::arg("activation") = "silu_gelu",
             py::arg("tie_embeddings") = false,
             py::arg("device") = "cuda")
        .def("load_gguf", &PyModel::load_gguf,
             py::arg("path"),
             py::arg("device") = "cuda")
        .def("load_auto", &PyModel::load_auto,
             py::arg("path"),
             py::arg("device") = "cuda")
        .def("load_vision_weights", [](PyModel& self, const std::string& mmproj_path, const std::string& device_str) {
        ...
        .def("generate", &PyModel::generate,
             ...
             py::arg("kv_cache_dtype") = "fp32",
             py::arg("gpu_layers") = -1)
        .def("generate_stream", &PyModel::generate_stream,
             py::arg("prompt_ids"),
             py::arg("callback"),
             py::arg("max_new_tokens") = 256,
             py::arg("temperature") = 1.0f,
    ...
    py::class_<PyInferenceContext>(m, "InferenceContext")
        .def("forward", &PyInferenceContext::forward,
             py::arg("input_ids"),
             py::arg("start_pos") = 0)
        .def("forward_with_embeddings", &PyInferenceContext::forward_with_embeddings,
             py::arg("embeddings"),
             py::arg("start_pos") = 0)
        .def("get_embeddings", &PyInferenceContext::get_embeddings,
             py::arg("input_ids"))
