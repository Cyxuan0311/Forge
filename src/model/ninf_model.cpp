...
private:
    ...
    void parse_object(std::unordered_map<std::string, std::string>& out, const std::string& prefix) {
        ++pos_;
        skip_ws();
        if (pos_ < src_.size() && src_[pos_] == '}') { ++pos_; return; }
    ...
    std::string parse_primitive() {
        ...
        while (pos_ < src_.size() && src_[pos_] != ',' && src_[pos_] != '}' &&
               src_[pos_] != ']' && !std::isspace(static_cast<unsigned char>(src_[pos_]))) {
            ++pos_;
        }
    ...
    std::string parse_array() {
        ...
        while (pos_ < src_.size() && depth > 0) {
            if (src_[pos_] == '[') ++depth;
            else if (src_[pos_] == ']') { --depth; if (depth == 0) break; }
            result += src_[pos_];
            ++pos_;
        }
        if (pos_ < src_.size()) ++pos_;
...
static DataType dtype_from_uint32(uint32_t d) {
    switch (d) {
        ...
        case 4: return DataType::Q4_K;
        case 5: return DataType::INT8;
...
bool NinfModel::load(const std::string& path) {
    ...
    if (header_.meta_size > 0) {
        metadata_ = std::string(
            static_cast<const char*>(mapped_data_) + header_.meta_offset,
            header_.meta_size
        );
    }
...
std::string NinfModel::get_metadata_str(const std::string& key, const std::string& default_val) const {
    if (metadata_.empty()) return default_val;
    ...
    if (it != parsed_meta_.end()) return it->second;
    return default_val;
}
...
std::vector<int32_t> NinfModel::get_metadata_int_array(const std::string& key, const std::vector<int32_t>& default_val) const {
