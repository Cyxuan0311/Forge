...
static void print_usage(const char* prog) {
    std::cout <<
        ...
        "\n"
        "  # Text completion\n"
        "  " << prog << " -m model.gguf -p \"What is machine learning?\"\n"
        "\n"
        "  # CPU-only inference\n"
        "  " << prog << " -m model.gguf -ngl 0\n"
        "\n"
        "  # Multimodal inference\n"
        "  " << prog << " -m model.gguf --mmproj mmproj.gguf --image photo.jpg -p \"Describe this image\"\n"
        "\n"
        "  # Model info\n"
...
CliArgs parse_args(int argc, char** argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-h" || arg == "--help") {
            ...
            if (++i >= argc) { std::cerr << "Error: " << arg << " requires an argument\n"; std::exit(1); }
