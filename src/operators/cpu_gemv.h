...
static inline __m256 q4_0_block_partial_avx2(
    const float* a_row, int base,
    const uint8_t* qs, const __m128i lo_mask, const __m128i eight)
{
    ...
    __m256 partial = _mm256_mul_ps(
        _mm256_loadu_ps(a_row + base),
    ...
    partial = _mm256_fmadd_ps(
        ...
        partial);
    partial = _mm256_fmadd_ps(
        _mm256_loadu_ps(a_row + base + 16),
        _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(q_hi_signed)),
        partial);
    partial = _mm256_fmadd_ps(
...
static void gemv_q4_0_transB_avx2_q8(
    const float* a, const uint8_t* w, float* out,
    int M, int K, int N);

static void gemv_q4_0_transB_avx2(
    const float* a, const uint8_t* w, float* out,
    int M, int K, int N)
{
    ...
    #define Q4_0_PROCESS_BLOCK(a_row, w_row, bi, acc) do {                      \
        const uint8_t* _blk = (w_row) + (size_t)(bi) * 18;                      \
        uint16_t _sb; memcpy(&_sb, _blk, 2);                                    \
        __m256 _sc = fp16_to_fp32_broadcast_avx2(_sb);                          \
        __m256 _pa = q4_0_block_partial_avx2(a_row, (bi) * 32, _blk + 2,       \
                                              lo_mask, eight);                   \
        acc = _mm256_fmadd_ps(_sc, _pa, acc);                                   \
    } while(0)

    // Helper: prefetch the weight block for iteration bi
    #define Q4_0_PREFETCH_BLOCK(w_row, bi) do {                                 \
    ...
    if (M == 1) {
        ...
        for (int n = 0; n < N; n += NR) {
            ...
            const uint8_t* w_row1 = (rows > 1) ? w + (size_t)(n + 1) * blocks_per_row * BLOCK_BYTES : nullptr;
            const uint8_t* w_row2 = (rows > 2) ? w + (size_t)(n + 2) * blocks_per_row * BLOCK_BYTES : nullptr;
            const uint8_t* w_row3 = (rows > 3) ? w + (size_t)(n + 3) * blocks_per_row * BLOCK_BYTES : nullptr;

            if (full_blocks > 0) {
                Q4_0_PREFETCH_BLOCK(w_row0, 0);
                if (rows > 1) Q4_0_PREFETCH_BLOCK(w_row1, 0);
                if (rows > 2) Q4_0_PREFETCH_BLOCK(w_row2, 0);
                if (rows > 3) Q4_0_PREFETCH_BLOCK(w_row3, 0);
            }
            if (full_blocks > 1) {
                Q4_0_PREFETCH_BLOCK(w_row0, 1);
                if (rows > 1) Q4_0_PREFETCH_BLOCK(w_row1, 1);
                if (rows > 2) Q4_0_PREFETCH_BLOCK(w_row2, 1);
                if (rows > 3) Q4_0_PREFETCH_BLOCK(w_row3, 1);
            }

            int bi = 0;
            for (; bi + 1 < full_blocks; bi += 2) {
                Q4_0_PROCESS_BLOCK(a, w_row0, bi, acc0_a);
                if (rows > 1) Q4_0_PROCESS_BLOCK(a, w_row1, bi, acc1_a);
                if (rows > 2) Q4_0_PROCESS_BLOCK(a, w_row2, bi, acc2_a);
                if (rows > 3) Q4_0_PROCESS_BLOCK(a, w_row3, bi, acc3_a);

                if (bi + 2 < full_blocks) {
                    Q4_0_PREFETCH_BLOCK(w_row0, bi + 2);
                    if (rows > 1) Q4_0_PREFETCH_BLOCK(w_row1, bi + 2);
                    if (rows > 2) Q4_0_PREFETCH_BLOCK(w_row2, bi + 2);
                    if (rows > 3) Q4_0_PREFETCH_BLOCK(w_row3, bi + 2);
                }

                Q4_0_PROCESS_BLOCK(a, w_row0, bi + 1, acc0_b);
                if (rows > 1) Q4_0_PROCESS_BLOCK(a, w_row1, bi + 1, acc1_b);
                if (rows > 2) Q4_0_PROCESS_BLOCK(a, w_row2, bi + 1, acc2_b);
                if (rows > 3) Q4_0_PROCESS_BLOCK(a, w_row3, bi + 1, acc3_b);
            }
                    ...
                    auto proc_partial = [&](const uint8_t* w_row, __m256& acc_val) {
                        ...
                        if (exponent == 0) {
                            scale_f = mantissa == 0 ? 0.0f :
                                (sign ? -1.0f : 1.0f) * std::ldexp((float)mantissa / 1024.0f, -14);
                        } else {
                            scale_f = (sign ? -1.0f : 1.0f) * std::ldexp(
                                1.0f + (float)mantissa / 1024.0f, (int)exponent - 15);
                        }
                        const uint8_t* qs = block + 2;
                        for (int j = 0; j < 16 && base + j < K; ++j) {
                            float qv = static_cast<float>((qs[j] & 0x0F) - 8) * scale_f;
                            acc_val = _mm256_fmadd_ps(_mm256_set1_ps(a[base + j]), _mm256_set1_ps(qv), acc_val);
                        }
                        for (int j = 0; j < 16 && base + 16 + j < K; ++j) {
                            float qv = static_cast<float>(((qs[j] >> 4) & 0x0F) - 8) * scale_f;
                            acc_val = _mm256_fmadd_ps(_mm256_set1_ps(a[base + 16 + j]), _mm256_set1_ps(qv), acc_val);
                        }
                    };
                    proc_partial(w_row0, acc0);
                    if (rows > 1) proc_partial(w_row1, acc1);
                    if (rows > 2) proc_partial(w_row2, acc2);
                    if (rows > 3) proc_partial(w_row3, acc3);
                }
            }

            out[n + 0] = hsum_avx2(acc0);
            if (rows > 1) out[n + 1] = hsum_avx2(acc1);
            if (rows > 2) out[n + 2] = hsum_avx2(acc2);
            if (rows > 3) out[n + 3] = hsum_avx2(acc3);
        }
    } else {
                ...
                if (full_blocks < blocks_per_row) {
                    ...
                    if (remaining > 0) {
                        ...
                        for (int j = 0; j < 16 && base + 16 + j < K; ++j) {
                            float qv = static_cast<float>(((qs[j] >> 4) & 0x0F) - 8) * scale_f;
                            acc = _mm256_fmadd_ps(
                                _mm256_set1_ps(a_row[base + 16 + j]),
                                _mm256_set1_ps(qv), acc);
                        }
                    }
                }
...
static void gemv_fp32_transB_avx2(
    ...
    if (M == 1) {
        ...
        for (int n = 0; n < N; n += NR) {
            ...
            auto finalize = [&](const float* a_r, const float* b_r, __m256 acc_val) -> float {
                ...
                for (int k = (K / 8) * 8; k < K; ++k) sum += a_r[k] * b_r[k];
                return sum;
            };
            out[n + 0] = finalize(a_row, b_row0, acc0);
            if (rows > 1) out[n + 1] = finalize(a_row, b_row1, acc1);
            if (rows > 2) out[n + 2] = finalize(a_row, b_row2, acc2);
            if (rows > 3) out[n + 3] = finalize(a_row, b_row3, acc3);
        }
    } else {
...
#ifdef USE_AVX2
static void gemv_q8_0_transB_avx2(
    const float* a, const uint8_t* w, float* out,
    int M, int K, int N)
{
    constexpr int BLOCK_SIZE = 32;
    ...
    auto process_block = [&](const float* a_row, const uint8_t* w_row, int bi, __m256& acc) {
        ...
        __m256 partial = _mm256_mul_ps(
            _mm256_loadu_ps(a_row + base),
            _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(qs)))));
        partial = _mm256_fmadd_ps(
            _mm256_loadu_ps(a_row + base + 8),
            _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(qs + 8)))),
            partial);
        partial = _mm256_fmadd_ps(
            _mm256_loadu_ps(a_row + base + 16),
            _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(qs + 16)))),
            partial);
        partial = _mm256_fmadd_ps(
            _mm256_loadu_ps(a_row + base + 24),
            _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64(reinterpret_cast<const __m128i*>(qs + 24)))),
            partial);
    ...
    if (M == 1) {
        ...
        for (int n = 0; n < N; n += NR) {
            ...
            for (int bi = 0; bi < blocks_per_row; ++bi) {
                int base = bi * BLOCK_SIZE;
                if (K - base < BLOCK_SIZE) break;

                process_block(a_row, w_row0, bi, acc0);
                if (rows > 1) process_block(a_row, w_row1, bi, acc1);
                if (rows > 2) process_block(a_row, w_row2, bi, acc2);
                if (rows > 3) process_block(a_row, w_row3, bi, acc3);
            }
            ...
            // Handle partial last block
            {
                ...
                if (remaining > 0 && remaining < BLOCK_SIZE) {
                    ...
                    if (rows > 1 && w_row1) process_partial(w_row1, acc1);
                    if (rows > 2 && w_row2) process_partial(w_row2, acc2);
                    if (rows > 3 && w_row3) process_partial(w_row3, acc3);
                }
            }
...
// ---- Q4_1 fused GEMV ----
// Q4_1 block: 2 bytes scale (fp16) + 2 bytes min (fp16) + 16 bytes quants = 20 bytes per 32 elements
// Dequantization: value = q * d + m  (q: 4-bit unsigned 0-15, d: scale, m: min offset)
//
...
static inline void q4_1_block_partial_avx2(
    const float* a_row, int base,
    const uint8_t* qs,
...
static void gemv_q4_1_transB_avx2(
    const float* a, const uint8_t* w, float* out,
    int M, int K, int N)
{
    constexpr int BLOCK_SIZE = 32;
    ...
    #define Q4_1_PROCESS_BLOCK(a_row, w_row, bi, acc_a, acc_b) do {               \
        const uint8_t* _blk = (w_row) + (size_t)(bi) * 20;                        \
        uint16_t _db, _mb; memcpy(&_db, _blk, 2); memcpy(&_mb, _blk + 2, 2);     \
    ...
    #define Q4_1_PREFETCH_BLOCK(w_row, bi) do {                                   \
    ...
    if (M == 1) {
        ...
        for (int n = 0; n < N; n += NR) {
            ...
            if (rows > 1) Q4_1_PREFETCH_BLOCK(w_row1, 0);
            if (rows > 2) Q4_1_PREFETCH_BLOCK(w_row2, 0);
            if (rows > 3) Q4_1_PREFETCH_BLOCK(w_row3, 0);
            ...
            for (; bi + 1 < full_blocks; bi += 2) {
                ...
                if (rows > 1) Q4_1_PROCESS_BLOCK(a_row, w_row1, bi, acc1_p, acc1_m);
                if (rows > 2) Q4_1_PROCESS_BLOCK(a_row, w_row2, bi, acc2_p, acc2_m);
                if (rows > 3) Q4_1_PROCESS_BLOCK(a_row, w_row3, bi, acc3_p, acc3_m);

                // Prefetch 2 blocks ahead
                if (bi + 2 < full_blocks) {
                    Q4_1_PREFETCH_BLOCK(w_row0, bi + 2);
                    if (rows > 1) Q4_1_PREFETCH_BLOCK(w_row1, bi + 2);
                    if (rows > 2) Q4_1_PREFETCH_BLOCK(w_row2, bi + 2);
                    if (rows > 3) Q4_1_PREFETCH_BLOCK(w_row3, bi + 2);
                }

                // Block bi+1 -> same acc_p / acc_m (dual-chain via FMA latency hiding)
                Q4_1_PROCESS_BLOCK(a_row, w_row0, bi + 1, acc0_p, acc0_m);
                if (rows > 1) Q4_1_PROCESS_BLOCK(a_row, w_row1, bi + 1, acc1_p, acc1_m);
                if (rows > 2) Q4_1_PROCESS_BLOCK(a_row, w_row2, bi + 1, acc2_p, acc2_m);
                if (rows > 3) Q4_1_PROCESS_BLOCK(a_row, w_row3, bi + 1, acc3_p, acc3_m);
            }
                ...
                if (remaining > 0) {
                    auto process_partial = [&](const uint8_t* w_row, __m256& acc_ref) {
                        ...
                        for (int j = 0; j < 16 && base + j < K; ++j) {
                            ...
                            acc_ref = _mm256_fmadd_ps(_mm256_set1_ps(a_row[base + j]), _mm256_set1_ps(qv), acc_ref);
                        }
                        for (int j = 0; j < 16 && base + 16 + j < K; ++j) {
                            float qv = static_cast<float>((qs[j] >> 4) & 0xF) * d_val + m_val;
                            acc_ref = _mm256_fmadd_ps(_mm256_set1_ps(a_row[base + 16 + j]), _mm256_set1_ps(qv), acc_ref);
                        }
                    };
                    process_partial(w_row0, acc0);
                    if (rows > 1 && w_row1) process_partial(w_row1, acc1);
            ...
            for (int m_idx = 0; m_idx < M; ++m_idx) {
                ...
                if (full_blocks < blocks_per_row) {
                    ...
                    if (remaining > 0) {
                        ...
                        for (int j = 0; j < 16 && base + 16 + j < K; ++j) {
                            float qv = static_cast<float>((qs[j] >> 4) & 0xF) * d_val + m_val;
                            acc = _mm256_fmadd_ps(_mm256_set1_ps(a_row[base + 16 + j]), _mm256_set1_ps(qv), acc);
                        }
                    }
                }
...
static void quantize_row_q8_0(const float* src, block_q8_0* dst, int k) {
    ...
    for (int bi = 0; bi < nb; ++bi) {
        ...
        for (int j = n_el; j < QK8_0; ++j) dst[bi].qs[j] = 0;
    }
}
...
static inline __m256 dot_q4_0_q8_0_block_avx2(
    const uint8_t* w_block, const block_q8_0& q8)
{
    uint16_t ws; memcpy(&ws, w_block, 2);
    ...
    int M, int K, int N)
{
    ...
    for (int n = 0; n < N; n += NR) {
        ...
        for (int bi = 0; bi < full_blocks; ++bi) {
            acc0 = _mm256_add_ps(acc0, dot_q4_0_q8_0_block_avx2(w_row0 + bi * BLOCK_BYTES, q8_act[bi]));
            if (rows > 1) acc1 = _mm256_add_ps(acc1, dot_q4_0_q8_0_block_avx2(w_row1 + bi * BLOCK_BYTES, q8_act[bi]));
            if (rows > 2) acc2 = _mm256_add_ps(acc2, dot_q4_0_q8_0_block_avx2(w_row2 + bi * BLOCK_BYTES, q8_act[bi]));
            if (rows > 3) acc3 = _mm256_add_ps(acc3, dot_q4_0_q8_0_block_avx2(w_row3 + bi * BLOCK_BYTES, q8_act[bi]));
        }
        ...
        if (full_blocks < blocks_per_row) {
            ...
            if (remaining > 0) {
                auto process = [&](const uint8_t* row, __m256& acc) {
                    ...
                    const uint8_t* qs = block + 2;
                    for (int j = 0; j < 16 && base + j < K; ++j) {
                        float qv = (float)((int)(qs[j] & 0x0F) - 8) * scale_f * q8_act[full_blocks].qs[j];
                        acc = _mm256_fmadd_ps(_mm256_set1_ps(a[base + j]), _mm256_set1_ps(qv), acc);
                    }
                    for (int j = 0; j < 16 && base + 16 + j < K; ++j) {
                        float qv = (float)((int)((qs[j] >> 4) & 0x0F) - 8) * scale_f * q8_act[full_blocks].qs[16 + j];
                        acc = _mm256_fmadd_ps(_mm256_set1_ps(a[base + 16 + j]), _mm256_set1_ps(qv), acc);
                    }
                };
                process(w_row0, acc0);
                if (rows > 1 && w_row1) process(w_row1, acc1);
                if (rows > 2 && w_row2) process(w_row2, acc2);
                if (rows > 3 && w_row3) process(w_row3, acc3);
            }
        }
...
#ifdef USE_AVX2
static void gemv_q4_0_fused_qkv_avx2(
    const float* a,
    const uint8_t* wq,
    const uint8_t* wk,
    const uint8_t* wv,
    ...
    int K, int N_q, int N_k, int N_v)
{
    ...
    #define FQKV_PROCESS_BLOCK(a_row, w_row, bi, acc) do {               \
        const uint8_t* _blk = (w_row) + (size_t)(bi) * BLOCK_BYTES;      \
        uint16_t _sb; memcpy(&_sb, _blk, 2);                             \
        __m256 _sc = fp16_to_fp32_broadcast_avx2(_sb);                   \
        __m256 _pa = q4_0_block_partial_avx2(a_row, (bi) * BLOCK_SIZE,   \
                                              _blk + 2, lo_mask, eight); \
        acc = _mm256_fmadd_ps(_sc, _pa, acc);                            \
    } while(0)

    #define FQKV_PREFETCH_BLOCK(w_row, bi) do {                          \
    ...
    auto process_row = [&](const float* a_row, const uint8_t* w_row, float* out, int N_out) {
        ...
        for (int n = 0; n < N_out; ++n) {
            ...
            if (full_blocks > 0) FQKV_PREFETCH_BLOCK(row, 0);
            if (full_blocks > 1) FQKV_PREFETCH_BLOCK(row, 1);

            int bi = 0;
            for (; bi + 1 < full_blocks; bi += 2) {
                FQKV_PROCESS_BLOCK(a_row, row, bi, acc_a);
                if (bi + 2 < full_blocks) FQKV_PREFETCH_BLOCK(row, bi + 2);
...
#ifdef USE_AVX2
static void gemv_q4_0_fused_ffn_up_avx2(
    const float* a,
    ...
    int K, int N)
{
    ...
    #define FFU_PROCESS_BLOCK(a_row, w_row, bi, acc) do {                 \
        const uint8_t* _blk = (w_row) + (size_t)(bi) * BLOCK_BYTES;      \
        uint16_t _sb; memcpy(&_sb, _blk, 2);                             \
        __m256 _sc = fp16_to_fp32_broadcast_avx2(_sb);                   \
        __m256 _pa = q4_0_block_partial_avx2(a_row, (bi) * BLOCK_SIZE,   \
    ...
    #define FFU_PREFETCH_BLOCK(w_row, bi) do {                            \
        const uint8_t* _pblk = (w_row) + (size_t)(bi) * BLOCK_BYTES;     \
        _mm_prefetch((const char*)_pblk, _MM_HINT_T0);                   \
    } while(0)
...
static void gemv_q4_0_ffn_down_residual_avx2(
    const float* a,
    const uint8_t* w,
    ...
    int K, int N)
{
    ...
    #define FFDR_PROCESS_BLOCK(a_row, w_row, bi, acc) do {                \
        const uint8_t* _blk = (w_row) + (size_t)(bi) * BLOCK_BYTES;      \
        uint16_t _sb; memcpy(&_sb, _blk, 2);                             \
        __m256 _sc = fp16_to_fp32_broadcast_avx2(_sb);                   \
        __m256 _pa = q4_0_block_partial_avx2(a_row, (bi) * BLOCK_SIZE,   \
    ...
    #define FFDR_PREFETCH_BLOCK(w_row, bi) do {                           \
        const uint8_t* _pblk = (w_row) + (size_t)(bi) * BLOCK_BYTES;     \
        _mm_prefetch((const char*)_pblk, _MM_HINT_T0);                   \
    } while(0)
    ...
    for (int n = 0; n < N; n += NR) {
        ...
        if (full_blocks > 0) {
            ...
            if (rows > 1) FFDR_PREFETCH_BLOCK(w_row1, 0);
            if (rows > 2) FFDR_PREFETCH_BLOCK(w_row2, 0);
            if (rows > 3) FFDR_PREFETCH_BLOCK(w_row3, 0);
        }
        if (full_blocks > 1) {
            FFDR_PREFETCH_BLOCK(w_row0, 1);
            if (rows > 1) FFDR_PREFETCH_BLOCK(w_row1, 1);
            if (rows > 2) FFDR_PREFETCH_BLOCK(w_row2, 1);
            if (rows > 3) FFDR_PREFETCH_BLOCK(w_row3, 1);
        }

        int bi = 0;
        for (; bi + 1 < full_blocks; bi += 2) {
            FFDR_PROCESS_BLOCK(a, w_row0, bi, acc0_a);
            if (rows > 1) FFDR_PROCESS_BLOCK(a, w_row1, bi, acc1_a);
            if (rows > 2) FFDR_PROCESS_BLOCK(a, w_row2, bi, acc2_a);
            if (rows > 3) FFDR_PROCESS_BLOCK(a, w_row3, bi, acc3_a);

            if (bi + 2 < full_blocks) {
                FFDR_PREFETCH_BLOCK(w_row0, bi + 2);
                if (rows > 1) FFDR_PREFETCH_BLOCK(w_row1, bi + 2);
                if (rows > 2) FFDR_PREFETCH_BLOCK(w_row2, bi + 2);
                if (rows > 3) FFDR_PREFETCH_BLOCK(w_row3, bi + 2);
            }

            FFDR_PROCESS_BLOCK(a, w_row0, bi + 1, acc0_b);
            if (rows > 1) FFDR_PROCESS_BLOCK(a, w_row1, bi + 1, acc1_b);
            if (rows > 2) FFDR_PROCESS_BLOCK(a, w_row2, bi + 1, acc2_b);
            if (rows > 3) FFDR_PROCESS_BLOCK(a, w_row3, bi + 1, acc3_b);
        }
        ...
        if (full_blocks < blocks_per_row) {
            ...
            if (remaining > 0) {
                ...
                proc(w_row0, acc0);
                if (rows > 1) proc(w_row1, acc1);
                if (rows > 2) proc(w_row2, acc2);
                if (rows > 3) proc(w_row3, acc3);
            }
        }

        out[n + 0] = hsum_avx2(acc0) + residual[n + 0];
        if (rows > 1) out[n + 1] = hsum_avx2(acc1) + residual[n + 1];
        if (rows > 2) out[n + 2] = hsum_avx2(acc2) + residual[n + 2];
        if (rows > 3) out[n + 3] = hsum_avx2(acc3) + residual[n + 3];
    }
...
static inline void q4_k_subblock_dot_avx2(
    const float* a_row, int base,
    const uint8_t* qs,
    const __m256 d_v, const __m256 dmin_v,
    float sc_even, float m_even,
    float sc_odd, float m_odd,
    __m256& acc_sc, __m256& acc_min)
{
    const __m128i lo_mask = _mm_set1_epi8(0x0F);
    ...
    __m256 a1 = _mm256_loadu_ps(a_row + base + 8);
    pl0 = _mm256_fmadd_ps(a1,
        _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_srli_si128(q_lo_0, 8))), pl0);
    ...
    pu0 = _mm256_fmadd_ps(a3,
        _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_srli_si128(q_hi_0, 8))), pu0);
    ...
    pl1 = _mm256_fmadd_ps(a5,
        _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_srli_si128(q_lo_1, 8))), pl1);
    ...
    pu1 = _mm256_fmadd_ps(a7,
        _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_srli_si128(q_hi_1, 8))), pu1);
...
static void gemv_q4_k_transB_prefill_avx2(
    const float* a, const uint8_t* w, float* out,
    int M, int K, int N)
{
        ...
        for (int m = 0; m < M; ++m) {
            ...
            for (int bi = 0; bi < full_sbs; ++bi) {
                ...
                for (int _j = 0; _j < 4; ++_j) {
                    ...
                    q4_k_subblock_dot_avx2(a_row, _sub_base, _qs,
                        _dv, _dmv, _sc_e, _m_e, _sc_o, _m_o, acc_sc, acc_min);
                    _qs += 32;
                    _is += 2;
                }
            }
...
static void gemv_q4_k_transB_avx2(
    const float* a, const uint8_t* w, float* out,
    int M, int K, int N)
{
    constexpr int SB_SIZE = 256;
    ...
    #define Q4K_PROCESS_SB(a_row, w_row, bi, acc_sc, acc_min) do {                \
        const uint8_t* _sb = (w_row) + (size_t)(bi) * 144;                        \
        uint16_t _db, _dmb; memcpy(&_db, _sb, 2); memcpy(&_dmb, _sb + 2, 2);      \
        __m256 _dv = fp16_to_fp32_broadcast_avx2(_db);                             \
        __m256 _dmv = fp16_to_fp32_broadcast_avx2(_dmb);                           \
    ...
    #define Q4K_PREFETCH_SB(w_row, bi) do {                                        \
    ...
    if (M == 1) {
        ...
        for (int n = 0; n < N; n += NR) {
            ...
            if (rows > 1) Q4K_PREFETCH_SB(w_row1, 0);
            if (rows > 2) Q4K_PREFETCH_SB(w_row2, 0);
            if (rows > 3) Q4K_PREFETCH_SB(w_row3, 0);

            for (int bi = 0; bi < full_sbs; ++bi) {
                Q4K_PROCESS_SB(a_row, w_row0, bi, acc0_sc, acc0_min);
                if (rows > 1) Q4K_PROCESS_SB(a_row, w_row1, bi, acc1_sc, acc1_min);
                if (rows > 2) Q4K_PROCESS_SB(a_row, w_row2, bi, acc2_sc, acc2_min);
                if (rows > 3) Q4K_PROCESS_SB(a_row, w_row3, bi, acc3_sc, acc3_min);

                // Prefetch next super-block
                if (bi + 1 < full_sbs) {
                    Q4K_PREFETCH_SB(w_row0, bi + 1);
                    if (rows > 1) Q4K_PREFETCH_SB(w_row1, bi + 1);
                    if (rows > 2) Q4K_PREFETCH_SB(w_row2, bi + 1);
                    if (rows > 3) Q4K_PREFETCH_SB(w_row3, bi + 1);
                }
            }
...
#ifdef USE_AVX2
static void gemv_q4_k_fused_ffn_up_avx2(
    const float* a, const uint8_t* w_gate, const uint8_t* w_up, float* out,
    int K, int N)
{
    constexpr int SB_SIZE = 256;
    constexpr int SB_BYTES = 144;
    ...
    #define Q4K_FFU_PROCESS_SB(a_row, w_row_g, w_row_u, bi, g_sc, g_mn, u_sc, u_mn) do { \
        const uint8_t* _sb_g = (w_row_g) + (size_t)(bi) * 144;                           \
        const uint8_t* _sb_u = (w_row_u) + (size_t)(bi) * 144;                           \
        uint16_t _db_g, _dmb_g, _db_u, _dmb_u;                                           \
        memcpy(&_db_g, _sb_g, 2); memcpy(&_dmb_g, _sb_g + 2, 2);                         \
        memcpy(&_db_u, _sb_u, 2); memcpy(&_dmb_u, _sb_u + 2, 2);                         \
        ...
        for (int _j = 0; _j < 4; ++_j) {                                                  \
            ...
            q4_k_subblock_dot_avx2(a_row, _sub_base, _qs_g,                               \
                _dv_g, _dmv_g, _sc_e_g, _m_e_g, _sc_o_g, _m_o_g, g_sc, g_mn);            \
            q4_k_subblock_dot_avx2(a_row, _sub_base, _qs_u,                               \
                _dv_u, _dmv_u, _sc_e_u, _m_e_u, _sc_o_u, _m_o_u, u_sc, u_mn);            \
            _qs_g += 32; _qs_u += 32; _is += 2;                                           \
        }                                                                                  \
    } while(0)
    ...
    for (int n = 0; n < N; ++n) {
        ...
        for (int bi = 0; bi < full_sbs; ++bi) {
            Q4K_FFU_PROCESS_SB(a, gate_row, up_row, bi,
                acc_g_sc, acc_g_mn, acc_u_sc, acc_u_mn);
        }
...
#ifdef USE_AVX2
static void gemv_q4_K_fused_qkv_avx2(
    const float* a,
        ...
        for (int n = 0; n < N_out; ++n) {
            ...
            for (int bi = 0; bi < full_sbs; ++bi) {
                ...
                for (int j = 0; j < 4; ++j) {
                    ...
                    q4_k_subblock_dot_avx2(a, sub_base, qs,
                        d_v, dm_v, sc_e, m_e, sc_o, m_o, acc_sc, acc_mn);
                    qs += 32;
                    is += 2;
                }
            }
...
static inline void q6_k_dot_16_avx2(
    const float* a_row, int base,
...
static inline void q6_k_process_sb_avx2(
    const float* a_row, int k_offset,
    const uint8_t* sb,
    __m256& acc)
{
    uint16_t d_bits;
    ...
    for (int sub = 0; sub < 2; ++sub) {
        ...
        // Pair 1: q1[0..15] (ql low, qh bits[1:0], sc[0]) + q3[0..15] (ql high, qh bits[5:4], sc[4])
        ...
        // Pair 2: q2[0..15] (ql+32 low, qh bits[3:2], sc[2]) + q4[0..15] (ql+32 high, qh bits[7:6], sc[6])
        ...
        q6_k_dot_16_avx2(a_row, base + 16,   ql_cur + 16, qh_cur + 16, 0, true,  d_val * sc_cur[1], acc);
        // Pair 4: q3[16..31] (ql+16 high, qh+16 bits[5:4], sc[5])
        q6_k_dot_16_avx2(a_row, base + 80,   ql_cur + 16, qh_cur + 16, 4, false, d_val * sc_cur[5], acc);
        // Pair 5: q2[16..31] (ql+48 low, qh+16 bits[3:2], sc[3])
        q6_k_dot_16_avx2(a_row, base + 48,   ql_cur + 48, qh_cur + 16, 2, true,  d_val * sc_cur[3], acc);
        // Pair 6: q4[16..31] (ql+48 high, qh+16 bits[7:6], sc[7])
        q6_k_dot_16_avx2(a_row, base + 112,  ql_cur + 48, qh_cur + 16, 6, false, d_val * sc_cur[7], acc);
    }
}

static void gemv_q6_k_ffn_down_residual_avx2(
    const float* a, const uint8_t* w, const float* residual, float* out,
    int K, int N)
{
    constexpr int SB_SIZE = 256;
    ...
    for (int n = 0; n < N; n += NR) {
        ...
        for (int bi = 0; bi < full_sbs; ++bi) {
            ...
            if (rows > 1) q6_k_process_sb_avx2(a, k_off, w_row1 + (size_t)bi * SB_BYTES, acc1);
            if (rows > 2) q6_k_process_sb_avx2(a, k_off, w_row2 + (size_t)bi * SB_BYTES, acc2);
            if (rows > 3) q6_k_process_sb_avx2(a, k_off, w_row3 + (size_t)bi * SB_BYTES, acc3);
        }
