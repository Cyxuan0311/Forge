    ...
    int M, int K, int N)
{
    ...
    if (M > 1) {
        ...
        if ((int)_dequant_buf.size() < K) _dequant_buf.resize(K);
        ...
        for (int n = 0; n < N; ++n) {
            ...
            for (int bi = 0; bi < full_blocks; ++bi) {
                ...
                _mm256_storeu_ps(dq + base +  0, _mm256_mul_ps(
                    _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(q_lo_s)), _sc));
                _mm256_storeu_ps(dq + base +  8, _mm256_mul_ps(
                    _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_srli_si128(q_lo_s, 8))), _sc));
                _mm256_storeu_ps(dq + base + 16, _mm256_mul_ps(
                    _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(q_hi_s)), _sc));
                _mm256_storeu_ps(dq + base + 24, _mm256_mul_ps(
            ...
            if (full_blocks < blocks_per_row) {
                ...
                if (rem > 0) {
                    ...
                    if (_exp == 0) {
                        _sf = _man == 0 ? 0.0f :
                            (_sig ? -1 : 1) * std::ldexp(static_cast<float>(_man) / 1024.0f, -14);
                    } else {
                        _sf = (_sig ? -1 : 1) * std::ldexp(
                            1.0f + static_cast<float>(_man) / 1024.0f, static_cast<int>(_exp) - 15);
                    }
                    const uint8_t* qs = blk + 2;
        ...
        for (int n = 0; n < N; n += NR) {
            ...
            for (; bi + 1 < full_blocks; bi += 2) {
                ...
                if (rows > 1) Q4_0_PROCESS_BLOCK(a_row, w_row1, bi, acc1_a);
                if (rows > 2) Q4_0_PROCESS_BLOCK(a_row, w_row2, bi, acc2_a);
                if (rows > 3) Q4_0_PROCESS_BLOCK(a_row, w_row3, bi, acc3_a);
                ...
                if (rows > 1) Q4_0_PROCESS_BLOCK(a_row, w_row1, bi + 1, acc1_b);
                if (rows > 2) Q4_0_PROCESS_BLOCK(a_row, w_row2, bi + 1, acc2_b);
                if (rows > 3) Q4_0_PROCESS_BLOCK(a_row, w_row3, bi + 1, acc3_b);
            }
    ...
    int M, int K, int N)
{
    ...
    if (M > 1) {
        ...
        if ((int)_dq.size() < K) _dq.resize(K);
        float* dq = _dq.data();
        ...
        for (int n = 0; n < N; ++n) {
            ...
            for (int bi = 0; bi < blocks_per_row; ++bi) {
                ...
                if (remaining >= BLOCK_SIZE) {
                    const uint8_t* block = w_row + bi * BLOCK_BYTES;
                    uint16_t _sb; memcpy(&_sb, block, 2);
                    __m256 _sc = fp16_to_fp32_broadcast_avx2(_sb);
                    const int8_t* qs = reinterpret_cast<const int8_t*>(block + 2);

                    _mm256_storeu_ps(dq + base +  0, _mm256_mul_ps(
                        _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64((const __m128i*)qs))), _sc));
                    _mm256_storeu_ps(dq + base +  8, _mm256_mul_ps(
                        _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64((const __m128i*)(qs+8)))), _sc));
                    _mm256_storeu_ps(dq + base + 16, _mm256_mul_ps(
                        _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64((const __m128i*)(qs+16)))), _sc));
                    _mm256_storeu_ps(dq + base + 24, _mm256_mul_ps(
                        _mm256_cvtepi32_ps(_mm256_cvtepi8_epi32(_mm_loadl_epi64((const __m128i*)(qs+24)))), _sc));
                } else if (remaining > 0) {
                    const uint8_t* block = w_row + bi * BLOCK_BYTES;
                    uint16_t _sb; memcpy(&_sb, block, 2);
                    ...
                    float _sf;
                    if (_exp == 0) {
                        _sf = _man == 0 ? 0.0f : (_sig ? -1 : 1) * std::ldexp(static_cast<float>(_man)/1024.0f,-14);
                    } else {
                        _sf = (_sig ? -1 : 1) * std::ldexp(1.0f+static_cast<float>(_man)/1024.0f, (int)_exp-15);
                    }
                    const int8_t* qs = reinterpret_cast<const int8_t*>(block + 2);
    ...
    int M, int K, int N)
{
    ...
    if (M > 1) {
        ...
        for (int n = 0; n < N; ++n) {
            ...
            for (int bi = 0; bi < full_blocks; ++bi) {
                const uint8_t* blk = w_row + (size_t)bi * BLOCK_BYTES;
                uint16_t _db, _mb; memcpy(&_db, blk, 2); memcpy(&_mb, blk + 2, 2);
...
static inline float dot_q4_K_q8_K_avx2(
    ...
    int nb)
{
    ...
    for (int i = 0; i < nb; ++i) {
        ...
        for (int j = 0; j < QK_K / 64; ++j) {
            ...
            const __m256i q8l = _mm256_loadu_si256((const __m256i*)q8d); q8d += 32;
            __m256i p16l = _mm256_maddubs_epi16(q4l, q8l);
            p16l = _mm256_madd_epi16(scale_l, p16l);

            const __m256i q8h = _mm256_loadu_si256((const __m256i*)q8d); q8d += 32;
...
static inline float dot_q6_K_q8_K_avx2(
    const uint8_t* q6_row,
    const block_q8_K* q8,
    int nb)
{
    constexpr int QK_K = 256;
...
static void gemv_q6_K_transB_avx2(
    const float* a, const uint8_t* w, float* out,
    int M, int K, int N)
{
    constexpr int QK_K = 256;
