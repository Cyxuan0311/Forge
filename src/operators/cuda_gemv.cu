...
// ---- FFN Down Fused Q4_0 (M=1, decode) ----
// Computes: out = matmul_transB(silu_multiply_result, w2) + residual
// Fuses the down_proj GEMV with residual add, avoiding one extra kernel launch
// and one extra global memory write+read for the intermediate ffn_out.
//
// Optimization: Warp row-tiling (ROWS_PER_WARP rows per warp).
// Each warp reads ffn_mid once and reuses it for ROWS_PER_WARP output rows,
// reducing ffn_mid global memory reads by ROWS_PER_WARP x.
// For K=18944, N=3584, ROWS_PER_WARP=4:
//   Before: 3584 warps * 74KB = 264MB ffn_mid reads
//   After:   896 warps * 74KB =  66MB ffn_mid reads (4x reduction)
...
void launch_output_proj_q4_0(const float* x, const void* q_weight, float* out, int K, int N,
    ...
    if (warps_per_row < 1)
        warps_per_row = 1;
    if (warps_per_row > 16)
        warps_per_row = 16;  // Allow more warps for large K
