...
// =========================================================================
__device__ __forceinline__ const uint8_t* paged_kv_row(
    const uint8_t* base,
...
__device__ __forceinline__ uint8_t* paged_kv_row_mut(
    uint8_t* base,
    const int32_t* __restrict__ page_ids,
    void* const* __restrict__ page_ptrs,
    int page_size, size_t row_bytes, int j) {
...
__device__ __forceinline__ void quantize_row_q8_0(const float* __restrict__ src,
    ...
    for (int bi = tid; bi < num_blocks; bi += bs) {
        ...
        float d = amax / 127.0f;
        if (d == 0.0f) d = 1.0f;  // avoid div-by-zero; scale becomes irrelevant
        float id = 1.0f / d;
...
__global__ void kv_scatter_kernel(
...
struct RowFnF16 { __device__ void operator()(const float* s, uint8_t* d, int n) {
    quantize_row_f16(s, reinterpret_cast<__half*>(d), n);
}};
struct RowFnFP32 { __device__ void operator()(const float* s, uint8_t* d, int n) {
    copy_row_fp32(s, reinterpret_cast<float*>(d), n);
}};
struct RowFnQ8_0 { __device__ void operator()(const float* s, uint8_t* d, int n) {
    quantize_row_q8_0(s, d, n);
}};
struct RowFnQ4_0 { __device__ void operator()(const float* s, uint8_t* d, int n) {
    quantize_row_q4_0(s, d, n);
}};
...
void launch_kv_scatter(
    ...
    case KVCacheDType::F16:
        kv_scatter_kernel<RowFnF16><<<n_tokens, threads, 0, stream>>>(
            k_src, v_src, k_page_ptrs, v_page_ptrs, page_ids,
            n_tokens, pos, page_size, kv_dim, k_row_bytes, v_row_bytes);
        break;
...
__global__ void paged_attn_q4_0_decode_kernel(
...
#pragma unroll
    ...
    for (int j = tid; j < kv_len; j += block_size) {
        ...
        const uint8_t* k_row = paged_kv_row(nullptr, page_ids, k_page_ptrs,
                                            page_size, q_row_size, j) + head_byte_offset;
        const uint8_t* v_row = paged_kv_row(nullptr, page_ids, v_page_ptrs,
                                            page_size, q_row_size, j) + head_byte_offset;
...
// ---- F16 paged decode ----
template <int HEAD_DIM, int NUM_WARPS>
__global__ void paged_attn_f16_decode_kernel(
    const float* __restrict__ Q,
    void* const* __restrict__ k_page_ptrs,
    void* const* __restrict__ v_page_ptrs,
    const int32_t* __restrict__ page_ids,
...
// ---- Q8_0 paged decode ----
template <int HEAD_DIM, int NUM_WARPS>
__global__ void paged_attn_q8_0_decode_kernel(
    const float* __restrict__ Q,
    void* const* __restrict__ k_page_ptrs,
    void* const* __restrict__ v_page_ptrs,
    const int32_t* __restrict__ page_ids,
...
void launch_paged_flash_attention_gqa_decode_q4_0(
    ...
    int page_size, size_t q_row_size, const float* mask_row,
    cudaStream_t stream) {
...
#define LAUNCH(HD)                                                             \
    paged_attn_q4_0_decode_kernel<HD, 4>                                       \
        <<<blocks, 128, 0, stream>>>(Q, k_page_ptrs, v_page_ptrs, page_ids,    \
                                     O, kv_len, num_heads, num_kv_heads,       \
                                     page_size, q_row_size, mask_row)
