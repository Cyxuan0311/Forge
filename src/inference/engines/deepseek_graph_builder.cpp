...
int DeepSeekGraphBuilder::build_layer_graph(ComputeGraph& graph,
                                              int hidden_idx,
    ...
    if (lw.layer_type == LayerType::MLA) {
        return build_mla_layer(graph, hidden_idx, lw, cfg, layer_idx, seq_len, start_pos, dev, kv_cache);
    } else {
        return build_gqa_layer(graph, hidden_idx, lw, cfg, layer_idx, seq_len, start_pos, dev, kv_cache);
    }
}

int DeepSeekGraphBuilder::build_gqa_layer(ComputeGraph& graph,
                                            int hidden_idx,
                                            const LayerWeights& lw,
                                            const ModelConfig& cfg,
                                            int layer_idx,
    ...
    int normed_idx = graph.add_node("attn_norm", OpType::RMS_NORM,
        {hidden_idx, attn_w_idx}, d_eps_params, dev);

    int wq_ds_idx = graph.add_input(lw.wq());
    std::vector<int> q_ds_inputs = {ref(normed_idx), wq_ds_idx};
    if (lw.bq()) q_ds_inputs.push_back(graph.add_input(lw.bq()));
    ...
    if (lw.bk()) k_ds_inputs.push_back(graph.add_input(lw.bk()));
    ...
    if (lw.bv()) v_ds_inputs.push_back(graph.add_input(lw.bv()));
    ...
    int proj_idx = graph.add_node("out_proj", OpType::MUL_MAT_TRANSB,
        {ref(attn_idx), wo_ds_idx}, nullptr, dev);
    ...
    int up_idx = graph.add_node("up_proj", OpType::MUL_MAT_TRANSB,
        {ref(ffn_normed_idx), w3_ds_idx}, nullptr, dev);

    int silu_idx = graph.add_node("silu_gate", OpType::SILU, {ref(gate_idx)},
        nullptr, dev);

    int ffn_mid_idx = graph.add_node("multiply_gate_up", OpType::MUL,
        {ref(silu_idx), ref(up_idx)}, nullptr, dev);
...
int DeepSeekGraphBuilder::build_mla_layer(ComputeGraph& graph,
                                            int hidden_idx,
                                            const LayerWeights& lw,
                                            const ModelConfig& cfg,
                                            int layer_idx,
    ...
    if (lw.wq_a() && lw.wq_b()) {
        ...
        q_latent_idx = graph.add_node("q_b_proj", OpType::MUL_MAT_TRANSB,
            {ref(q_a_idx), wq_b_idx}, nullptr, dev);
    } else {
    ...
    int k_rope_idx = graph.add_node("k_rope", OpType::ROPE, {ref(compressed_kv_idx)}, k_rope_mla_p, dev);
    delete[] k_rope_mla_p;

    // KV cache update
    int cache_idx = graph.add_node("cache_cpy", "cpy_k", {ref(k_rope_idx), ref(v_latent_idx)},
        [&kv_cache, layer_idx, seq_len, dev](const std::vector<TensorPtr>& inputs) -> TensorPtr {
            kv_cache.update(layer_idx, inputs[0], inputs[1], seq_len);
            if (kv_cache.kv_dtype() == KVCacheDType::Q4_0) {
                kv_cache.dequantize_layer(layer_idx);
    ...
    int k_all_idx = graph.add_node("get_k", "get_k", {ref(cache_idx)},
        [&kv_cache, layer_idx, dev](const std::vector<TensorPtr>&) -> TensorPtr {
            return kv_cache.get_key(layer_idx);
        }, dev);

    int v_all_idx = graph.add_node("get_v", "get_v", {ref(cache_idx)},
        [&kv_cache, layer_idx, dev](const std::vector<TensorPtr>&) -> TensorPtr {
            return kv_cache.get_value(layer_idx);
        }, dev);

    int total_len_idx = graph.add_node("total_len", "custom", {ref(cache_idx)},
    ...
    int k_sliced_idx = graph.add_node("k_slice", "view", {ref(k_all_idx), ref(total_len_idx)},
    ...
    int v_sliced_idx = graph.add_node("v_slice", "view", {ref(v_all_idx), ref(total_len_idx)},
        [&kv_cache, layer_idx](const std::vector<TensorPtr>& inputs) -> TensorPtr {
            auto v_all = inputs[0];
            int total = kv_cache.filled(layer_idx);
            int max_seq = kv_cache.max_seq_len();
    ...
    int k_final_idx = (dev == DeviceType::CUDA) ?
        graph.add_node("k_to_cuda", "cpy", {ref(k_sliced_idx)},
            [dev](const std::vector<TensorPtr>& inputs) -> TensorPtr {
                auto t = inputs[0];
                if (t->device() == DeviceType::CPU) {
                    auto t_cuda = std::make_shared<Tensor>(DataType::FP32, t->shape(), DeviceType::CUDA);
                    t_cuda->copy_from(*t);
                    return t_cuda;
                }
                return t;
    ...
    int v_final_idx = (dev == DeviceType::CUDA) ?
        graph.add_node("v_to_cuda", "cpy", {ref(v_sliced_idx)},
            [dev](const std::vector<TensorPtr>& inputs) -> TensorPtr {
                auto t = inputs[0];
                if (t->device() == DeviceType::CPU) {
                    auto t_cuda = std::make_shared<Tensor>(DataType::FP32, t->shape(), DeviceType::CUDA);
    ...
    int attn_idx = graph.add_node("attention", "flash_attn_gqa",
        {ref(q_rope_idx), ref(k_final_idx), ref(v_final_idx)},
        [num_heads, kv_lora_rank, dev]
        (const std::vector<TensorPtr>& inputs) -> TensorPtr {
            ...
            return ops::scaled_dot_product_attention_2d(
                q_rope, k_sliced, v_sliced, seq_len_q, total_len, num_heads, kv_lora_rank, true);
        }, dev);
...
int DeepSeekGraphBuilder::build_output_graph(ComputeGraph& graph,
                                               int hidden_idx,
    ...
    int norm_idx = graph.add_node("output_norm", OpType::RMS_NORM,
        {hidden_idx, out_norm_w_idx}, out_eps_ds_params);
    ...
    int logits_idx = graph.add_node("output_proj", OpType::MUL_MAT_TRANSB,
        {ref(norm_idx), out_w_idx}, nullptr);

    return logits_idx;
}

// Register DeepSeek graph builders
static GraphBuilderAutoRegister _reg_deepseek_gb("deepseek", []() -> std::unique_ptr<LayerGraphBuilder> {
    return std::make_unique<DeepSeekGraphBuilder>();
});
static GraphBuilderAutoRegister _reg_deepseek_v2_gb("deepseek_v2", []() -> std::unique_ptr<LayerGraphBuilder> {
    return std::make_unique<DeepSeekGraphBuilder>();
});
static GraphBuilderAutoRegister _reg_deepseek_v3_gb("deepseek_v3", []() -> std::unique_ptr<LayerGraphBuilder> {
    return std::make_unique<DeepSeekGraphBuilder>();
});
