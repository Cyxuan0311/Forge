...
Prerequisites:
  1. Download GGUF model:
     aria2c -x 16 -s 16 -d /mnt/g/AI -o DeepSeek-R1-Distill-Qwen-7B-Q4_K_M.gguf \
...
Usage:
  ...
  # Other options:
  python3 examples/deepseek_r1_inference.py --device cpu --gpu-layers 0
  python3 examples/deepseek_r1_inference.py --no-stream
  python3 examples/deepseek_r1_inference.py --max-new-tokens 512 --temperature 0.6
  python3 examples/deepseek_r1_inference.py --verify-tokenizer
  python3 examples/deepseek_r1_inference.py --profile          # Enable C++ profiler + Python timing
...
class PerfTimer:
    ...
    def print_summary(self):
        ...
        print("=" * 80)
        print(f"{'Stage':<35} {'Count':>6} {'Total(ms)':>10} {'Avg(ms)':>10} {'Min(ms)':>10} {'Max(ms)':>10} {'%Total':>7}")
        print("-" * 80)
        ...
        for name, rec in sorted_items:
            ...
            print(f"{name:<35} {rec['count']:>6} {rec['total_ms']:>10.2f} {avg:>10.3f} {rec['min_ms']:>10.3f} {rec['max_ms']:>10.3f} {pct:>6.1f}%")

        print("-" * 80)
        print(f"{'TOTAL':<35} {'':>6} {grand_total:>10.2f} {'':>10} {'':>10} {'':>10} {'100.0%':>7}")
        print("=" * 80)
...
def apply_chat_template(tokenizer, messages, add_generation_prompt=True):
    ...
    Supports DeepSeek-R1 format:
       system:    <BOS>{system_prompt}
       user:      <｜User｜>{content}
       assistant: <｜Assistant｜>{content}<｜end▁of▁sentence｜>
       generation prompt: <｜Assistant｜>

    Special tokens are inserted as single token IDs to ensure
    correct encoding. Text segments are encoded individually.
    """
    ...
    for msg in messages:
        ...
        if role == "system":
            # System prompt goes right after BOS, no special marker
        ...
        if len(token_buffer) >= 4 or token_id == eos_token_id:
            try:
                ...
                if token_id == eos_token_id:
                    # Flush on EOS even if decode fails
                    text = tokenizer.decode(token_buffer, skip_special=True,
                                            strip_leading_space=False)
                    print(text, end="", flush=True)
                    token_buffer.clear()
    ...
    while True:
        ...
        if args.no_stream:
            ...
            if ctx is not None:
                ...
                gc.collect()
            if profiling_enabled:
                perf.start("context.create")
            ctx = model.create_context(
                kv_cache_dtype=args.kv_cache_dtype,
                gpu_layers=args.gpu_layers,
            )
            if profiling_enabled:
                perf.stop("context.create")
...
def parse_args():
    ...
    parser.add_argument("--temperature", type=float, default=0.6,
                        help="Sampling temperature (lower for more deterministic)")
...
def main():
    ...
    if profiling_enabled:
        perf.start("startup.tokenizer_load")
    tokenizer = load_tokenizer(model_path)
    if profiling_enabled:
        perf.stop("startup.tokenizer_load")
    ...
    if profiling_enabled:
        perf.start("startup.context_create")
    ctx = model.create_context(
        kv_cache_dtype=args.kv_cache_dtype,
        gpu_layers=args.gpu_layers,
    )
    if profiling_enabled:
        perf.stop("startup.context_create")
