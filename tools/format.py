...
def pack_header(meta_offset, meta_size, tensor_count, data_size):
    return struct.pack(
        HEADER_FMT, NINF_MAGIC, NINF_VERSION, meta_offset, meta_size, tensor_count, data_size
    )
...
def read_ninf_header(filepath):
    with open(filepath, "rb") as f:
        ...
        magic, version, meta_off, meta_sz, tensor_cnt, data_sz = struct.unpack_from(
            HEADER_FMT, header_data, 0
        )
        ...
        for _ in range(tensor_cnt):
            ...
            name_b, dtype, ndim, d0, d1, d2, d3, offset, size = struct.unpack_from(
                TENSOR_INFO_FMT, raw, 0
            )
