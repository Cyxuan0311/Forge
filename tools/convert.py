...
def convert_gguf_to_ninf(gguf_path, ninf_path, target_dtype="q4_0"):
    ...
    print(
        f"Model config: vocab={vocab_size}, hidden={hidden_dim}, "
    ...
    for t in tensors_info:
        ...
        if gguf_name == "output.weight" and dtype_ggml not in (
            ...
            print(
                f"  Skipping {gguf_name} ({ggml_type_name(dtype_ggml)}) "
        ...
        if dtype_ggml == GGML_TYPE_Q4_0:
            ...
            numel = int(np.prod(dims))
            print(f"  Dequantizing {ggml_type_name(dtype_ggml)} tensor {name} to FP32")
            fp32_data = dequantize(data, data_offset, dtype_ggml, dims)
            ninf_tensors.append((name, NINF_DTYPE_FP32, out_dims, fp32_data.tobytes()))

        dtype_out = (
            "Q4_0"
            if ninf_tensors[-1][1] == NINF_DTYPE_Q4_0
            else "Q4_1"
            if ninf_tensors[-1][1] == NINF_DTYPE_Q4_1
            else "FP32"
        )
        print(f"  {name}: {ggml_type_name(dtype_ggml)} -> {dtype_out} shape={ninf_tensors[-1][2]}")
